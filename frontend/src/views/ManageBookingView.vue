...
<template>
  ...
  >
    <v-card
      class="w-75 ma-3 pa-12 rounded-lg d-flex flex-column justify-space-around"
      height="660"
      elevation="3"
    ...
    >
      ...
      <v-card-actions class="d-flex justify-center" v-if="order.deleted == 0">
        ...
        >
          ...
          <v-card>
            <v-form
              @submit.prevent="orders.actions.addUpdate(orders.pickup, order)"
            >
              <v-card-title primary-title> Izmena adrese </v-card-title>
              <v-card-text>
                <v-text-field
                  name="Adresa polaska"
              ...
              <v-card-actions>
                ...
                <v-btn color="error" @click="orders.addressDialog = false"
                  >Zatvori</v-btn
                >
              </v-card-actions>
            </v-form>
        ...
        >
          ...
          <v-card>
            <v-form>
              <v-card-title primary-title> Izaberi broj mesta </v-card-title>
              <v-card-text>
                <v-number-input
              ...
              <v-card-actions>
                ...
                >
                  <template v-slot:activator="{ props: activatorPropsC }">
                    <v-btn color="success" v-bind="activatorPropsC">
                      Potvrdi
                    </v-btn>
                  </template>
                  <v-card>
                    ...
                    <v-card-actions>
                      <v-btn
                        ...
                        >Prihvati</v-btn
                      >
                      <v-btn color="error" @click="orders.clsSeats"
                        >Odustani</v-btn
                      >
                    </v-card-actions>
                  </v-card>
                ...
                <v-btn color="error" @click="orders.clsSeats">Zatvori</v-btn>
              </v-card-actions>
            </v-form>
          </v-card>
        </v-dialog>
        ...
        >
          ...
          <v-card>
            <v-form>
              <v-card-title primary-title> Izaberi novi datum </v-card-title>
              <v-card-text>
                <v-date-input
                  ...
                  <template #day="{ date }">
                    <div
                      :class="[
                        ...
                        'v-size-default',
                        {
                          'bg-red-darken-2 text-white pointer-events-none':
                            search.allowedDays.fullyBooked.includes(date),
                          'opacity-50 pointer-events-none':
                            !search.isDateAllowed(date),
                        },
                      ]"
                    >
                  ...
                  <template #day="{ date }">
                    <div
                      :class="[
                        ...
                        {
                          'bg-red-darken-2 text-white pointer-events-none':
                            search.allowedDaysIn.fullyBooked.includes(date),
                          'opacity-50 pointer-events-none':
                            !search.isDateInAllowed(date),
                        },
                      ]"
                    >
                      {{ new Date(date).getDate() }}
                    ...
                    <v-card-text class="w-75 d-flex justify-space-evenly">
                      ...
                      <div class="pa-3">
                        ...
                        <p class="ma-3">
                          Novi datum:
                          <span style="color: #00c853">{{
                            orders.requestDateInView
                          }}</span>
                        </p>
                        <p class="ma-3">
                          Trenutni datum:
                          <span style="color: #ec407a">{{
                            orders.currentDateIn
                          }}</span>
                        </p>
                      </div>
                    </v-card-text>
        ...
        >
          ...
          <v-card>
            <v-card-title primary-title>
              <p style="color: #d50000">
                Da li ste sigurni da želite da obrišete ovu vožnju?
              </p>
            </v-card-title>
            <v-card-actions>
              <v-btn color="success" @click="orders.actions.cancel"
                >Potvrdi</v-btn
              >
              <v-btn color="error" @click="orders.deleteDeny">Odustani</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-card-actions>
      <v-card-actions
        v-if="order.deleted == 1"
        class="d-flex justify-center align-center"
      >
        <v-btn variant="elevated" color="red-darken-4" width="30%"
          >Obrisano</v-btn
        >
      </v-card-actions>
    </v-card>
